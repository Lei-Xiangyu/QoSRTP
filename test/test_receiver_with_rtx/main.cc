...
static const struct {
  ...
  std::string cname = "test_receiver_with_rtx";
  std::string media_session_name = "test";
} global_config;

class ReceiverWithRtx : qosrtp::MediaSessionCallback {
 public:
  ReceiverWithRtx() : qosrtp_session_(nullptr) {
    qosrtp_session_ = ConstructQosrtpSession();
    if (!qosrtp_session_) {
      std::cout << "Failed to construct qosrtp session" << std::endl;
    }
  }
  ~ReceiverWithRtx() = default;
...
int main(int argc, char* argv[]) {
  ...
  ReceiverWithRtx* test_receiver_with_rtx = new ReceiverWithRtx();
  std::cout << "Any key will exit!" << std::endl;
  std::cin.get();
  delete test_receiver_with_rtx;
